...
const { selectAiAction, getActionKey, getActionSetHash, getSelectableCardIds, getAvailableActions, getMyPlayerState, createBotContext } = require('./util');
...
let gamesPlayed = 0;
// In self-play mode bot1 and bot2 both receive the 'gamestate' end-of-game
// event for the same underlying game, so without dedup gamesPlayed (and the
// TRAIN_EVERY_N trigger) would advance twice per actual game played.
const countedGameIds = new Set();
function countGameOnce(gameId) {
  if (countedGameIds.has(gameId)) return false;
  countedGameIds.add(gameId);
  if (countedGameIds.size > 2000) {
    // trim oldest half to keep this bounded over a long run
    const arr = Array.from(countedGameIds);
    countedGameIds.clear();
    for (const g of arr.slice(-1000)) countedGameIds.add(g);
  }
  return true;
}
...
function createSocket(id, name) {
  return new Promise((resolve, reject) => {
    ...
    sock.on('disconnect', (reason) => {
      ...
      reconnectTimer = setTimeout(restartOnce, 5000);
    });
...
async function runBot(id, name) {
  ...
  // Per-bot state (failed-action tracking, model cache, retry counters).
  // Previously this lived in bot/util.js module-level globals, which were
  // shared between bot1 and bot2 when self-play runs both in one process.
  ...
  async function handleGameState(data) {
    ...
    if (currentHash === lastStateHash && tried) {
      const player = getMyPlayerState(data, id, ctx);
      ...
      if (isDistributePrompt) {
        ...
        const action = await selectAiAction(data, SELF_PLAY_MODE, id, ctx);
        if (action) {
...
async function runTraining() {
  ...
  try {
    ...
    // loadGameRecordings pre-filters by file mtime before parsing JSON, so
    // this no longer re-reads every past game off disk on every training run.
    let recordings = await loadGameRecordings(cutoff);

    // Belt-and-suspenders: mtime is a proxy for completedAt, so double check
    // with the real field in case a file was touched without changing content.

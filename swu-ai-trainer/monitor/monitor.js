...
const BIND_HOST = process.env.MONITOR_BIND_HOST || '127.0.0.1';
const API_KEY = process.env.MONITOR_API_KEY || '';

// Safely single-quote a value for embedding in a *remote* shell command run
// over SSH. Any value that reaches sshCmd()'s `cmd` string by interpolation
// (bot names, deck names, etc.) must go through this — previously values
// like a bot name were spliced into `'{"name":"${name}"}'` unescaped, so a
// name containing a single quote or `$(...)` could run arbitrary commands
// on the remote host as the SSH user.
...
async function driverCurl(path) {
  const url = shellEscape(`http://localhost:${DRIVER_PORT}${path}`);
  return await sshCmd(`curl -sf ${url} 2>/dev/null || echo '___DRIVER_DOWN___'`);
}
...
async function driverRemoveBot(name) {
  // Build the JSON payload in JS (so it's correctly escaped for JSON), then
  // base64-encode it and pipe the decoded bytes straight into curl's stdin.
  // Base64 output only ever contains [A-Za-z0-9+/=], none of which are shell
  // metacharacters, so this is safe to interpolate directly — unlike trying
  // to re-embed the decoded JSON (which contains literal quotes) inside a
  // quoted shell argument.
  const payload = Buffer.from(JSON.stringify({ name })).toString('base64');
  const r = await sshCmd(
    `echo ${payload} | base64 -d | curl -sf -X POST http://localhost:${DRIVER_PORT}/api/bots/remove -H 'Content-Type: application/json' -d @- 2>/dev/null || echo '___DRIVER_DOWN___'`
  );
  if (!r.ok || r.data === '___DRIVER_DOWN___') {
    driverOk = false;
...
const server = http.createServer(async (req, res) => {
  ...
  // Auth gate: every /api/* route can add, remove, or reconfigure bots and
  // decks (and, via SSH, touch files on the Ubuntu box), so require the key
  // whenever one is configured. Without a configured key this relies solely
  // on BIND_HOST defaulting to localhost.
  if (API_KEY && url.pathname.startsWith('/api/') && req.headers['x-api-key'] !== API_KEY) {
...
if (!API_KEY) {
  log('WARNING: MONITOR_API_KEY is not set — the dashboard API is unauthenticated.');
  if (BIND_HOST !== '127.0.0.1' && BIND_HOST !== 'localhost') {
    log(`WARNING: MONITOR_BIND_HOST=${BIND_HOST} is not localhost — this API (which can run arbitrary SSH commands on your server via bot/deck actions) is reachable by anyone who can reach this host on port ${PORT}. Set MONITOR_API_KEY.`);
  }
}

server.listen(PORT, BIND_HOST, () => {
  log(`SWU Bot Monitor started at http://${BIND_HOST}:${PORT}`);
